<template>
  <div class="container">
    <v-card class="table" v-if="tournament" dark color="primary" max-width="80%" style="margin-right: auto; margin-left: auto; margin-bottom: 0;" >
    ...
    >
      ...
      >
        ...
        <template v-slot:item="{ item }">
          <tr v-if="tournamentResults.indexOf(item) + 1 === 1" style="height: 100px; background: rgb(233, 211, 98);   background: linear-gradient(62deg, rgba(233, 211, 98, 1) 0%, rgba(51, 51, 51, 0.1) 100%);">
            <td>{{ tournamentResults.indexOf(item) + 1 }}</td>
            <td>🏆</td>
            <td style="text-align: left; font-size: 1.1rem">
              {{ item.enrolledStudentUsername }}
            </td>
            <td>{{ item.numberOfCorrectAnswers }}</td>
            <td>{{ item.timeTaken }}</td>
          </tr>
          <tr v-else-if="tournamentResults.indexOf(item) + 1 === 2" style="height: 100px; background: rgb(169, 169, 169);   background: linear-gradient(62deg, rgba(169, 169, 169, 1) 0%, rgba(51, 51, 51, 0.1) 100%);">
            <td>{{ tournamentResults.indexOf(item) + 1 }}</td>
            <td></td>
            <td style="text-align: left; font-size: 1.1rem">
              {{ item.enrolledStudentUsername }}
          ...
          <tr v-else-if="tournamentResults.indexOf(item) + 1 === 3" style="height: 100px; background: rgb(205, 127, 50);   background: linear-gradient(62deg, rgba(205, 127, 50, 1) 0%, rgba(51, 51, 51, 0.1) 100%);">
            <td>{{ tournamentResults.indexOf(item) + 1 }}</td>
            <td></td>
            <td style="text-align: left; font-size: 1.1rem">
              {{ item.enrolledStudentUsername }}
